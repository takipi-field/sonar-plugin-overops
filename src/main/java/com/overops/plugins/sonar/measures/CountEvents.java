...
public class CountEvents {
    ...
    //prepares map so no Metrics<Integer> are null which causes sonar to fail a scan
...
//This method calculates how many times the event with the same class name and type happen only counts
    public HashMap<String, HashMap<String, Integer>> countClassErrors() {
        ...
        for (EventResult result : events.events) {
            //classes first event in list ex result.error_location.class_name= COM.EMPIRE.SHOPPINGCART.MANAGER.CUSTOMEVENTMANAGER reduced to just classname no extension
            ...
            if (!classErrorCounts.containsKey(eventClassNameShortened)) {
                ...
                //iterative variable has the same class as one already found and if it has the same type of error increment count
